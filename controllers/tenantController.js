...
module.exports.applyForUnit = async (req, res) => {
  try {
    ...
    if (!tenant || tenant.role !== "TENANT")
      return res
        .status(403)
        .json({ message: "Only tenants can apply for units" });
    ...
    res
      .status(201)
      .json({ message: `Tenant ${tenant.name} applied for unit #${unitId}` });
  } catch (error) {
...
module.exports.signLease = async (req, res) => {
  try {
    ...
    res.json({
      message: `Lease #${leaseId} signed successfully by ${tenant.name}`,
      lease,
    });
  } catch (error) {
    res.status(500).json({ message: "Server Error", error });
  ...
  try {
    ...
    if (String(lease.tenantId) !== String(tenantId))
      return res.status(403).json({
        message: "You can only request termination of your own lease",
      });

    if (lease.status !== "ACTIVE")
      return res
        .status(400)
        .json({ message: "Only ACTIVE leases can be requested to terminate" });

    if (lease.termination && lease.termination.status === "REQUESTED")
      return res
        .status(400)
        .json({ message: "You already have a pending termination request" });
...
module.exports.payInvoice = async (req, res) => {
  try {
    ...
    await invoice.save();
    res.json({
      message: `Invoice #${invoiceId} paid successfully by ${tenant.name}`,
      invoice,
    });
  } catch (error) {
    res.status(500).json({ message: "Server Error", error });
